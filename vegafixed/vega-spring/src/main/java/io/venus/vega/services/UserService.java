...
public class UserService {
    ...
    private final CustomUserLoginRepository customUserLoginRepository;
    ...
    public LoginUserResponseResource login(final LoginUserRequestResource loginUserRequestResource) {
        final var user = this.customUserLoginRepository.findByEmailAndPassword(
                loginUserRequestResource.getEmail(),
                loginUserRequestResource.getPassword()
        );

        if (user.getStatus() == UserStatus.CREATED || user.getStatus() == UserStatus.INACTIVE) {
            throw new BusinessException(HttpStatus.FORBIDDEN, "Account not activated");
        }

        final var authority = new SimpleGrantedAuthority(
